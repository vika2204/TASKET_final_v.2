...
class CommentController {
  ...
  static async getAllUserComments(req, res) {
    ...
    try {
      const comments = await CommentService.getAllUserComments(user_id);
      console.log(
        `Found ${comments.length} comments for user_id: ${user_id}`
      );
      res.status(200).json(comments);
    } catch (error) {
      console.error(
        `Error fetching comments for ticket_id: ${user_id}`,
        error
      );
      res.status(404).json({ error: error.message });
    }
